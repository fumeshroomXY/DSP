...
```
Since: **700 > 500**, the theorem is violated.
...
Fmax ​= 500 Hz
Fs​ = 1000 Hz
the signal period is: T ​= 2 ms
the sampling period is: Ts ​= 1 ms
```
...
```
Observation time: Twindow​ = 1 / resolution = 1.024 s
```
**The longer you observe the signal, the finer the frequency resolution becomes.**

Suppose you want to distinguish:

- 100.0 Hz
- 100.5 Hz

These two frequencies are only 0.5 Hz apart.

If your observation time is only 0.1 s:
```
resolution = Δf ≈ 10 Hz
```
they will look almost identical.

If your observation time is 2 s:
```
resolution = Δf ≈ 0.5 Hz
```
now the FFT can start separating them.

### A common misunderstanding

Many people think: "Higher sampling frequency gives better frequency resolution."
...
Fs = 10000 Hz
N = 1024
Observation time: 0.1024s
Resolution: 9.77Hz
```
The case samples much faster, but because it only covers about 0.1 seconds, **its frequency resolution is actually worse**.
